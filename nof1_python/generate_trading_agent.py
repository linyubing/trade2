...
class TradingAgent:
    ...
    def build_system_prompt(self, strategy: str = None) -> str:
        ...
        if strategy in ["ai-autonomous", "alpha-beta"]:
            ...
            system_prompt = (
                f"You are a helpful AI trading assistant. Strategy: {strategy}.\n"
                f"{risk_rules}"
                f"{protection_note}"
            )
            return system_prompt
...
'''
with open('D:/workFile/trade2/nof1_python/agent/trading_agent.py', 'w', encoding='utf-8') as f:
    f.write(content)

import os
size = os.path.getsize('D:/workFile/trade2/nof1_python/agent/trading_agent.py')
print(f'trading_agent.py written: {size} bytes')
